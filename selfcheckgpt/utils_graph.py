...
def binary_AUPRC(auprc_metric, inputs, targets):
    """
    Computes the binary average Area Under the Precision-Recall Curve (AUPRC).

    This function updates an AUPRC (Area Under the Precision-Recall Curve) metric
    using the predicted 'inputs' and true 'targets' and returns
    the biinary average AUPRC score.

    Args:
    - auprc_metric (torchmetrics.AveragePrecision): A PyTorch Metric object
      for computing the average precision.
    - inputs (tensor): Predicted probabilities or scores.
    - targets (tensor): True class labels.

    Returns:
    - binary_avg_auprc (float): average AUPRC score.
    """
    auprc_metric.reset()
    auprc_metric.update(inputs, targets)
    return auprc_metric.compute()


def macro_AUPRC(auprc_metric, inputs, targets, num_classes=3):
    """
    ...
    """
    auprc_metric.reset()
    auprc_metric.update(torch.nn.functional.one_hot(inputs, num_classes=num_classes), targets)
    return auprc_metric.compute()
